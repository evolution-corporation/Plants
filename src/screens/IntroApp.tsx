...
const ChangePage = (): animationValue => {
  ...
  const [state, updateStatusAnimation] = useReducer((state: animationState, value: LoadingStatus): animationState => {
  ...
  const startAnimation = () => Animated.timing(state.animation, {
    toValue: 1,
    duration: 600,
    useNativeDriver: false,
  }).start(()=>{updateStatusAnimation(LoadingStatus.READY)});
...
const IntroApp: FC<IntroAppProps> =  ({ onPress }: IntroAppProps) => {
  ...
  return (
    <Background style={styles.background} useNavigator={false}>
      <Animated.View
        style={[
          styles.logo,
          {
      ...
      <View style={{ width: '100%', alignItems: 'center' }}>
        <Animated.View style={[styles.introView, { opacity: hide, bottom: 40 }]}>
          <Text style={[styles.textIntro, { fontSize: 28 }]}>
          {i18n.t('f5c18015-15f7-4d64-a05f-0fae8b64d1d8')}
          </Text>
        </Animated.View>
        <Animated.View style={[styles.introView, { opacity: show }]}>
          <Text style={[styles.textIntro, { fontSize: 24 }]} >
            {i18n.t('03d9d661-8143-4617-a330-ad14e9865493')}
          </Text>
          <Image source={require('~assets/image/PeoplePlantASeedlingOnThebackgroundOfAMarker.png')} style={[styleImage.imageFullWidth]}/>
        </Animated.View>
      </View>
