...
export default class User implements UserData {
	...
	public static async checknUniqueNickName(nickName: string): Promise<boolean> {
		const collection: FirebaseFirestoreTypes.QuerySnapshot = await this.colection
	...
	private async updateDate(data: FirebaseFirestoreTypes.DocumentSnapshot): Promise<void> {
		...
		if (documentData) {
			...
			if (documentData.birthday instanceof firestore.Timestamp) {
				this.birthday = documentData.birthday.toDate();
			} else {
				await this.editData({ birthday: new Date(documentData.birthday), id: this.id });
				console.log('edit');
				return;
			}
